            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr><td colSpan="6" className="p-8 text-center text-gray-500">No expiring batches found.</td></tr>
                ) : (
                  data?.map((item, idx) => (
                    <tr key={idx} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className={`p-4 text-sm font-bold text-right ${item.days_left < 0 ? 'text-red-500' : 'text-yellow-500'}`}>
                        {item.days_left}
                      </td>
                      <td className="p-4 text-sm text-white text-right">{item.stock}</td>
                      <td className="p-4">
                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${
                            item.status === 'Expired' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'
                        }`}>
                            {item.status}
                        </span>
                      </td>
