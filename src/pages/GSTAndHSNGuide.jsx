...
export default function GSTAndHSNGuide() {
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-black to-gray-900 text-white p-6 md:p-12">
      ...
      {signupModal.open && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-900 rounded-lg p-8 max-w-md w-full border border-white/20">
            <h2 className="text-2xl font-bold mb-4">Join Cenvora</h2>
            <p className="text-gray-400 mb-6">
              {signupModal.hsnCode && `HSN Code ${signupModal.hsnCode} will be pre-filled in your invoices.`}
              {signupModal.gstRate && `GST Rate ${signupModal.gstRate}% will be pre-selected for applicable items.`}
            </p>
            <button
              onClick={() => window.location.href = '/signup'}
              className="w-full px-4 py-3 bg-purple-500 hover:bg-purple-600 rounded-lg font-semibold transition-all mb-3"
            >
              Sign Up Now
            </button>
            <button
              onClick={() => setSignupModal({ ...signupModal, open: false })}
              className="w-full px-4 py-3 bg-white/10 hover:bg-white/20 rounded-lg font-semibold transition-all"
            >
              Close
            </button>
          </div>
